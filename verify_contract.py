...
PASSWORD = os.getenv("API_USER_PASSWORD", "password")
...
def main():
    ...
    try:
        ...
        if not token:
            failures.append("Login failed: 'token' not found in response.")
        else:
    ...
    if not headers:
        ...
        print(f"Result: FAILED. Login failed, cannot proceed with other tests.")
        for f in failures:
            print(f"  - {f}")
        sys.exit(1)
